...
import { ListWorkoutExercisesUseCase } from "./list-workout-exercises.use-case.js";

describe("ListWorkoutExercisesUseCase", () => {
  it("should return exercises when day belongs to user plan", async () => {
    const exercises: WorkoutExerciseResult[] = [
      {
        id: "ex-1",
        name: "Supino",
        order: 0,
    ...
    const exerciseRepository: WorkoutExerciseRepository = {
      create: vi.fn(),
      findByDayId: vi.fn().mockResolvedValue(exercises),
      findByIdAndDayId: vi.fn(),
      update: vi.fn(),
      delete: vi.fn(),
      reorder: vi.fn(),
    };
    const useCase = new ListWorkoutExercisesUseCase(
      planRepository,
      dayRepository,
      exerciseRepository,
    );
    ...
    expect(exerciseRepository.findByDayId).toHaveBeenCalledWith("day-1");
  });
  ...
  it("should throw DayNotFoundError when day does not exist or plan not owned by user", async () => {
    ...
    await expect(
      useCase.execute({ dayId: "day-1", userId: "user-1" }),
    ).rejects.toThrow(DayNotFoundError);
    expect(exerciseRepository.findByDayId).not.toHaveBeenCalled();
  });
});
